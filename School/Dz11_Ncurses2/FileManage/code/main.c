...
void show_dir(uchar* _absPath, WINDOW* win, u8* ddY)
{
  u16 count=0;
  ...
  mvwprintw(win, count+3, 1, "ddY:%d, ChangeWin-3(ChangeBuf):%d", *ddY, ChangeWin-3);
  mvwprintw(win, count+4, 1, "0  %s", absPath[0]);
  mvwprintw(win, count+5, 1, "1  %s", absPath[1]);
...
void new_dir(uchar* oldPath, uchar* move, u8 ddY)
{
  u16 count=0;
  ...
  {
    //chdir("..");
    uchar newPath[PATH_MAX];
    strcpy(newPath, oldPath);
    for(u16 t=strlen(newPath); t>1; t--)
    {
      if(newPath[t]!='/')
        newPath[t]=0;
      else// if(newPath[t]=='/')
    ...
    {
      for(u16 t=0; t<strlen(oldPath); t++)
        oldPath[t]=0;
      strcpy(oldPath, newPath);
    }
    */
...
{	
  /*
  absPath[0]="/home/alexandr";
  absPath[1]="/home/alexandr";
  */
  ...
  */
  ...
  chdir(absPath[0]);
  /*
  new_dir(absPath[0], ".", dY[0]);
  new_dir(absPath[1], ".", dY[1]);
  */
  ...
  while((ch = getch()) != KEY_BACKSPACE)
    ...
    switch(ch)
      ...
      case '\t'://TAB
        ...
        show_dir(/*&dir[ChangeWin-3],*/absPath[ChangeWin-3], my_wins[ChangeWin], &dY[ChangeWin-3]);  
