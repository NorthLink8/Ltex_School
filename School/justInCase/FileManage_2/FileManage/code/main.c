...
void show_dir(/*DIR** d,*/uchar* aPath, WINDOW* win, u8* ddY)
{
  u16 count=0;
  DIR* d=opendir(aPath);
  if(d==NULL)
  ...
  mvwprintw(win, count+3, 1, "%s", aPath);
  mvwprintw(win, count+2, 1, "%d", *ddY);
  mvwprintw(win, count+4, 1, "(O_O) %s", absPath[0]);
  mvwprintw(win, count+5, 1, "(^_^) %s", absPath[1]);
...
void new_dir(/*DIR** d,*/uchar* path, uchar* folder, u8 ddY)
{
  u16 count=0;
  //uchar cwd[PATH_MAX];
