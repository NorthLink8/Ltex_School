  ...
  while((ch=getch())!=KEY_BACKSPACE)
  {
    switch(ch)
    {
      ...
      case KEY_F(5):
        ...
        wclear(my_wins[4]);
        //ManagerInit();
        //mvwprintw(my_wins[0], LINES-4, 3, "%s", "HELLO");
        if(ChangeWin<=4)
          COPYfile(absPath[0], absPath[1], my_wins[0], LINES-4, 5, dY[ChangeWin-3]);
        else if(ChangeWin>=5)
          COPYfile(absPath[1], absPath[0], my_wins[0], LINES-4, 5, dY[ChangeWin-3]);
        //ManagerInit();
        show_dir(absPath[0], my_wins[3], &dY[0]);
        show_dir(absPath[1], my_wins[4], &dY[1]);
        wrefresh(my_wins[3]);
